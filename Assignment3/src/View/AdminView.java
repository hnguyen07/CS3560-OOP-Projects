...
public class AdminView {	
	...
	private void build() {		
		...
		// Create the "ID Verification" button
		JButton verifyIDButton = new JButton("ID Verification");
		verifyIDButton.setBounds(415, 360, 275, 55);
		changeFont(verifyIDButton);
		// Use the IDCheckerVisitor class to get the invalid user/group IDs and display them.
		verifyIDButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent event) {
				IDCheckerVisitor idcv = new IDCheckerVisitor();
				root.accept(idcv);
				display(idcv.getInvalidIDs());
			}
		});

		// Create the "Last Updated User" button
		JButton lastUpdatedUserButton = new JButton("Last Updated User");
		lastUpdatedUserButton.setBounds(702, 360, 275, 55);
		changeFont(lastUpdatedUserButton);
		// Use the LastUpdatedUserVisitor class to get the last updated UserID.
		lastUpdatedUserButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent event) {
				LastUpdatedUserVisitor luuv = new LastUpdatedUserVisitor();
				root.accept(luuv);
				display("Last updated user: " + luuv.getLastUpdatedUser());
			}
		});
